...
public class Create implements CommandExecutor, ConversationAbandonedListener{
    ...
    public Create(){
        conversationFactory = new ConversationFactory(ThemedBuildPlugin.getPluginInstance())
                .withLocalEcho(false)
                .withModality(true)
                .withFirstPrompt(new setTitle())
                .withTimeout(600)
                .thatExcludesNonPlayersWithMessage(ThemedBuildPlugin.prefix + "You must be a player to send this command");
    }
        ...
        if(cmd.getName().equalsIgnoreCase("theme")){
            if(args.length == 0){
                ...
                if(!DBmanager.plots.containsKey(p.getUniqueId().toString())) {
                    p.sendMessage(ThemedBuildPlugin.prefix + "You don't have claimed plots in current theme");
                    return true;
                }
                ...
                if(nplots > 0) {
                    for(Plot pl : plots) {
                        if(pl.isIn(p.getLocation())) {
                            pl.reset();
                            p.sendMessage(ThemedBuildPlugin.prefix + "Plot restored to original state");
                            return true;
                        }
                    }
                }
                p.sendMessage(ThemedBuildPlugin.prefix + "You can only reset your own plot");
                return true;
            }
                ...
                for(Map.Entry<String, ArrayList<Plot>> e : DBmanager.plots.entrySet()){
                    for(Plot plot : (ArrayList<Plot>) e.getValue().clone()){
                        if(plot.isIn(p.getLocation())) {
                            ...
                            p.sendMessage(ThemedBuildPlugin.prefix + "Plot unclaimed");
                            return true;
                        }
                    }
                }
                ...
                p.sendMessage(ThemedBuildPlugin.prefix + "This plot isn't claimed");
                return true;
            }
            else if(args[0].equalsIgnoreCase("new")&&p.hasPermission("plotmanager.create")){
                if(ThemedBuildPlugin.getPluginInstance().getConfig().getStringList("ValidWorlds").contains(p.getWorld().getName())){
                    //create new theme
                    p.sendMessage(ThemedBuildPlugin.prefix + "Generating...");
                    String tname = "";
                    ...
                    if(args[1].equals("-m") && args.length > 3){
                        ...
                        if(!DBmanager.modelExists(modelname)){
                            p.sendMessage(ThemedBuildPlugin.prefix + "Model '"+modelname+"' doesn't exist");
                            p.sendMessage(ThemedBuildPlugin.prefix + "To see available models, use /theme listmodels");
                            return true;
                        }
                    }
                    ...
                    tname = tname.trim();
                    if(tname.length() > 30) {
                        p.sendMessage(ThemedBuildPlugin.prefix + "Provided name is too long");
                        return true;
                    }
    ...
    ////                p.teleport(DBmanager.curr.getCent());
                    ...
                    p.sendMessage(ThemedBuildPlugin.prefix + "You crazy... you cant make plots here!!!!");
                    return true;
                }
            }
                ...
                if(args.length >= 2){
                    ...
                    p.sendMessage(ThemedBuildPlugin.prefix + "Empty model created");
                ...
                if(args.length >= 2){
                    if(!DBmanager.modelExists(args[1])){
                        p.sendMessage(ThemedBuildPlugin.prefix + "Model '"+args[1]+"' doesn't exist");
                        return true;
                    }
                    if(args[1].equals("default")){
                        p.sendMessage(ThemedBuildPlugin.prefix + "Default model can't be deleted");
                        return true;
                    }
                    if(DBmanager.currModel != null && args[1].equals(DBmanager.currModel.getName())){
                        p.sendMessage(ThemedBuildPlugin.prefix + "Model currently in use, can't be deleted");
                        return true;
                    }
                    DBmanager.deletePlotModel(args[1]);
                    p.sendMessage(ThemedBuildPlugin.prefix + "Model '"+args[1]+"' deleted");
                    return true;
                }
            }
                ...
                if(args.length >= 2){
                    if(DBmanager.IncompleteModel == null){
                        p.sendMessage(ThemedBuildPlugin.prefix + "You have to create model first");
                        return true;
                    }
                    ...
                    if(args[1].equals("1")){
                        ...
                        p.sendMessage(ThemedBuildPlugin.prefix + "Invalid argument: " + args[1]);
                        return true;
                    }
                    p.sendMessage(ThemedBuildPlugin.prefix + "Point set");
                    return true;
                }
            }
                ...
                if(DBmanager.IncompleteModel != null){
                    ...
                    p.sendMessage(ThemedBuildPlugin.prefix + "Please wait...");
                ...
                p.sendMessage(ThemedBuildPlugin.prefix + "Existing models:");
                String models = "";
                ...
                p.sendMessage(ThemedBuildPlugin.prefix + "Displaying help for " + ChatColor.DARK_GREEN + "/theme " + args[1]);
                ...
                p.sendMessage(ThemedBuildPlugin.prefix + "Theme URL set to: " + ChatColor.GRAY + args[1]);
                return true;
            }
        }
        return false;
    ...
    public void conversationAbandoned(ConversationAbandonedEvent cae) {
        throw new UnsupportedOperationException(ThemedBuildPlugin.prefix + "Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
